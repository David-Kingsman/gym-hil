...
class KukaWindowAssemblyEnv(FrankaGymEnv):
    ...
    def _get_window_state(self) -> Tuple[np.ndarray, np.ndarray, float, float]:
        ...
        # Check if window is vertical (Z-axis should NOT point in Z direction, i.e., not flat on ground)
        # When window is flat: Z-axis points up (Z direction), dot product with [0,0,1] is close to 1
        # When window is vertical: Z-axis points in -X or +X direction, dot product with [0,0,1] is close to 0
        z_is_horizontal = abs(np.dot(window_z_axis, np.array([0, 0, 1]))) < 0.5  # Relaxed threshold: Z-axis is not pointing up/down
        
        # Check if window is tilted (not perfectly vertical)
        # When window is vertical: X-axis and Y-axis should be in Y-Z plane (no X component, or very small)
        # If window is tilted: X-axis or Y-axis will have significant X component
        # For a vertical window: X-axis should be in Y-Z plane, so X-axis[0] should be close to 0
        # Similarly, Y-axis should be in Y-Z plane, so Y-axis[0] should be close to 0
        x_axis_x_component = abs(window_x_axis[0])  # X-axis's X component (should be ~0 for vertical window)
        y_axis_x_component = abs(window_y_axis[0])  # Y-axis's X component (should be ~0 for vertical window)
        # Also check if X/Y axes have significant Z component (indicating tilt around Y or X axis)
        x_axis_z_component = abs(window_x_axis[2])  # X-axis's Z component
        y_axis_z_component = abs(window_y_axis[2])  # Y-axis's Z component
        
        # Window is properly vertical if:
        # 1. Z-axis is horizontal (not pointing up/down)
        # 2. X-axis and Y-axis are in Y-Z plane (X component is very small, < 0.15)
        # 3. One of X/Y axes should be roughly horizontal (Z component close to 0), the other should be vertical (Z component close to 1)
        #    For a vertical window: one axis (width) is horizontal, one axis (height) is vertical
        #    So: min(x_axis_z, y_axis_z) should be close to 0, max(x_axis_z, y_axis_z) should be close to 1
        x_axis_in_yz_plane = x_axis_x_component < 0.15  # X-axis should be in Y-Z plane (very small X component)
        y_axis_in_yz_plane = y_axis_x_component < 0.15  # Y-axis should be in Y-Z plane (very small X component)
        # One axis should be horizontal (Z ≈ 0), one should be vertical (Z ≈ 1)
        min_z_component = min(x_axis_z_component, y_axis_z_component)
        max_z_component = max(x_axis_z_component, y_axis_z_component)
        axes_properly_oriented = (min_z_component < 0.2 and max_z_component > 0.8)  # One horizontal, one vertical
        ...
        # Tilt penalty: if window is tilted, reduce alignment score
        # Calculate tilt angle: how much the window deviates from vertical
        # Penalize based on:
        # 1. X/Y axes having X component (should be 0 for vertical window)
        # 2. X/Y axes not being properly oriented (one horizontal, one vertical)
        x_y_plane_deviation = max(x_axis_x_component, y_axis_x_component)  # Should be close to 0
        orientation_deviation = min(min_z_component, 1.0 - max_z_component)  # Should be close to 0
        max_tilt = max(x_y_plane_deviation * 3.0, orientation_deviation * 2.0)  # Weight X deviation more heavily
        tilt_penalty = 1.0 - min(max_tilt, 1.0)  # Penalty factor (0-1), 1.0 = no tilt, 0.0 = severe tilt
        
        # Alignment: use Z-axis alignment directly if window is vertical (not flat)
        # If window is still flat, alignment should be low
        # However, if position is very close (<5cm), we relax the alignment check
        # This handles cases where window is inserted but alignment calculation might be off
        if z_is_horizontal and is_properly_vertical:
            # Window is properly vertical, use Z-axis alignment with target normal
            # Apply tilt penalty to reduce reward for tilted windows
            alignment = z_alignment * tilt_penalty
        elif z_is_horizontal:
            # Window is vertical but tilted: reduce alignment significantly
            # Tilted windows should not get high reward
            alignment = z_alignment * tilt_penalty * 0.5  # Additional penalty for tilted windows
        else:
            # Window is still flat (Z-axis pointing up/down)
            # Flat windows should not get high reward, even if position is close
            if z_alignment > 0.3:  # Relaxed threshold: if Z-axis is somewhat aligned
                # Window is flat but somewhat aligned: give partial credit
                alignment = z_alignment * 0.7  # Reduce alignment score for flat window
            elif pos_dist < 0.03:  # If position is very close (<3cm), check if properly vertical
                # Position is very close, but only give high alignment if window is properly vertical
                # Note: if we're in this branch, window is flat, so is_properly_vertical should be False
                # But check anyway in case of edge cases
                if is_properly_vertical:
                    alignment = 0.85  # High alignment score when position is very close and properly vertical
                else:
                    alignment = z_alignment * 0.5  # Reduced alignment for flat/tilted windows even when close
            elif pos_dist < 0.05:  # If position is close (<5cm), relax alignment check
                # Position is close, but penalize flat/tilted windows
                if is_properly_vertical:
                    alignment = max(z_alignment * 0.5, 0.7)  # Minimum 0.7 alignment if position is close and properly vertical
                else:
                    alignment = z_alignment * 0.3  # Reduced alignment for flat/tilted windows
            else:
                alignment = 0.0
        ...
        if pos_dist < 0.05 and (alignment < 0.5 or not is_properly_vertical):
            ...
            print(f"  window_z_axis={window_z_axis}, z_is_horizontal={z_is_horizontal}, z_alignment={z_alignment:.4f}")
    ...
    def _compute_reward(self) -> float:
        ...
        if self.reward_type == "dense":
            ...
            # Success criteria: close to target position (within 4cm) and well aligned (>80%)
            # Note: Reward is 0.0 until BOTH conditions are met simultaneously
            # Relaxed thresholds for easier success detection
            # Position threshold relaxed to 4cm (40mm) to account for visual insertion
            # Alignment threshold relaxed to 80% to account for calculation issues
            is_success = (pos_dist < 0.04 and alignment > 0.80)
