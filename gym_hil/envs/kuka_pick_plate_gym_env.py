...
# KUKA iiwa14 home position
# With gripper quat=identity, joint6 needs to rotate to make gripper face downward
# joint6=-90° rotates wrist to point gripper downward
_KUKA_HOME = np.asarray((0, 0.785398, 0, -1.5708, 0, 0.785398, 0))
_CARTESIAN_BOUNDS = np.asarray([[0.0, -0.5, 0], [1.0, 0.5, 0.8]])  # bounding box for the robot (X: 0.2-0.8, Y: -0.3-0.3, Z: 0-0.5)
_SAMPLING_BOUNDS = np.asarray([[0.3, -0.15], [0.6, 0.15]])  # Updated upper X bound to 0.6 to match plate position


class KukaPickPlateGymEnv(FrankaGymEnv):
    """Environment for a KUKA iiwa14 robot with vacuum gripper picking up a plate.
    
    This environment is similar to PandaPickPlateGymEnv but uses a KUKA iiwa14 robot
    instead of a Franka Panda robot. The task is to use the vacuum gripper to suck up
    the plate and lift it to a certain height.
    """
    ...
    def __init__(
        ...
        # Use KUKA vacuum gripper XML by default
        if xml_path is None:
            xml_path = Path(__file__).parent.parent / "assets" / "kuka_pick_plate_scene.xml"
    ...
    def reset(self, seed=None, **kwargs) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        ...
        if self._random_plate_position:
            ...
            plate_xy = np.asarray([0.6, 0.0])  # Moved forward 10cm (from 0.5 to 0.6)
            self._data.jnt("plate_joint").qpos[:3] = (*plate_xy, self._plate_z)
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        # Apply the action to the robot (now using motor actuators like Panda, so standard gains work)
...
if __name__ == "__main__":
    ...
    env = KukaPickPlateGymEnv(render_mode="human")
