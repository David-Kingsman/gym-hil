...
# Extended X bounds to reach wall at X=0.7m (wall is further than block at X=0.5m)
_CARTESIAN_BOUNDS = np.asarray([[0.2, -0.3, 0], [0.8, 0.3, 0.5]])
_SAMPLING_BOUNDS = np.asarray([[0.3, -0.15], [0.5, 0.15]])

# New Target Definition (Coordinates from XML: x=0.7, y=0.0, z=0.225)
# Wall and base are positioned at X=0.7m (further than block at X=0.5m)
# Wall center is at Y=0.0 (aligned with base center)
_TARGET_POS = np.asarray([0.7, 0.0, 0.225]) 
...
class PandaMasonryBlockInsertionEnv(FrankaGymEnv):
    ...
    def __init__(
        ...
        # XML中moveable_block body的初始pos是 [0.5, 0.0, 0.06]，所以初始Z应该是0.06m
        try:
            ...
            self._block_z_init = 0.06  # XML中定义的初始Z
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        # 注意：target位置在X=0.7，所以边界检查应该允许block移动到target位置
        block_pos = self._data.sensor("block_pos").data
        target_pos = self._get_target_pos()
    ...
    def _compute_reward(self) -> float:
        ...
        # If no target_site (e.g., scene.xml), use PickCube-style reward
        if self._target_site_id < 0:
        ...
        # For masonry_insertion.xml: include placement reward
        target_pos = self._get_target_pos()
        dist_tcp_block = np.linalg.norm(block_pos - tcp_pos)
        r_close = np.exp(-15 * dist_tcp_block) * 0.5

        dist_block_target = np.linalg.norm(block_pos - target_pos)
        r_place = np.exp(-15 * dist_block_target) * 0.5
        
        # Only reward placement if block has been lifted or is close to the target Z
        if block_pos[2] > self._z_init + 0.01 or dist_block_target < 0.2:
            return float(np.clip(r_close + r_place, 0.0, 1.0))
        else:
            # If not yet lifted, just reward reaching the block
            return float(np.clip(r_close, 0.0, 1.0))
    ...
    def _is_success(self) -> bool:
        ...
        if self._has_gripper_joints:
            try:
                ...
                # Gripper打开时，joint角度应该 > 0.5（约50度），表示已释放
                # 使用0.5而不是0.3，确保gripper真正打开，而不是在放下过程中稍微打开
        ...
        try:
            ...
            is_stable = lin_vel < 0.01  # 线速度 < 1cm/s
